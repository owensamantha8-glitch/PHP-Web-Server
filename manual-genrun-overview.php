...
// -------------------------------------------------------------------
// STARTUP CHECKS: details go to the server error log only (no paths or commands on screen)
// -------------------------------------------------------------------
$conr_path = LUM_ROOT . "/Manual Readings/Manual GenRun Control/manual-genrun-conr.php";

if (!is_readable($conr_path)) {
    error_log('LUM manual-genrun-overview: required file not readable: ' . $conr_path);
    http_response_code(500);
    die("<div style='background:#121212; color:#ffb3b3; padding:20px; margin:20px; font-family:sans-serif;'>This page is temporarily unavailable. Please contact the system administrator.</div>");
}

lum_connect('manual'); // $manual_db_conn (bootstrap.php)
require_once($conr_path);

if (!class_exists('manual_genrun_conr')) {
    error_log('LUM manual-genrun-overview: class manual_genrun_conr not found in ' . $conr_path);
    http_response_code(500);
    die("<div style='background:#121212; color:#ffb3b3; padding:20px; margin:20px; font-family:sans-serif;'>This page is temporarily unavailable. Please contact the system administrator.</div>");
}
// -------------------------------------------------------------------
...
<head>
    ...
    <title>Generator Runtime Overview</title>
...
<div class="d-flex-wrapper">
    ...
    <div class="sidebar">
        ...
        <div class="nav flex-column nav-pills mb-5" role="tablist">
            <a href="manual-water-overview.php" class="nav-link">
                <i class="bi bi-droplet me-2"></i> Water Meters
            </a>
            <a href="manual-genrun-overview.php" class="nav-link active">
                <i class="bi bi-lightning-charge me-2"></i> Generator Runtime
            </a>
        </div>

        <form method="GET" action="manual-genrun-overview.php" class="bg-dark p-3 border border-secondary shadow-sm">
            <div class="mb-3">
            ...
            <div class="d-grid">
                <a href="manual-genrun-overview.php" class="btn btn-brand btn-sm px-2 shadow-sm">Clear Filters</a>
            </div>
        </form>
    </div>
    ...
    <div class="main-content">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom border-secondary" style="min-height: 31px;">
            <span class="fs-6 text-white mb-0">Generator Runtime Overview</span>
            <?php if (lum_can('manual_readings', 'edit')): ?>
            <a href="Manual GenRun Insert/manual-genrun-form.php" class="btn btn-danger btn-sm shadow-sm" style="background-color: #e3000f; border:none;">+ Add New Runtime</a>
            <?php endif; ?>
        </div>
                ...
                <table class="table table-dark table-hover table-borderless align-middle">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="text-end" style="width: 25%;">Runtime Hours</th>
                        </tr>
                    </thead>
                    <tbody>
                        <!-- Safety check to prevent fatal loops if the database query fails -->
                        <?php if (is_array($readings) && count($readings) > 0): ?>
                            <?php foreach ($readings as $row): ?>
                                <tr>
                                    ...
                                    <td class="text-end text-warning"><?php echo htmlspecialchars($row['reading_hours']); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="3" class="text-center py-5 text-muted">
                                    ...
                                    No generator runtime readings found matching your filters.
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
