...
if (!defined('LUM_BACKBILL_ENGINE')) {
    ...
    function lumAdjRecalculate(array $line, array $tenant_row, array $c) {
        ...
        $slip_engine_file = lum_resolve_path('/Tenant Management/Tenant Consumption Slips/slip-engine.php');
        if (!$slip_engine_file || !is_readable($slip_engine_file)) {
            throw new RuntimeException('The consumption slip engine is not available.');
        }
        require_once $slip_engine_file;
